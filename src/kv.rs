...
///
/// Pairs are persisted to disk in log files. Log files get monotonically
/// increasing numbers as names with a .log extension
/// In memory storage of the keys and values is provided by `BTreeMap`
///
/// ```rust
...
pub struct KvStore {
    // data directory
    path: PathBuf,
    // a map of log file numbers to their reader handle
    readers: HashMap<u64, BufReaderWithPos<File>>,
    // handle for the current file writer
    writer: BufWriterWithPos<File>,
    current_file: u64,
    index: BTreeMap<String, CommandPos>,
    // Number of bytes we can delete in compaction (i.e. stale/redundant commands)
    uncompacted: u64,
}

impl KvStore {
    /// Opens a new store with given path
    ///
    /// This will create a new directory if the given one does not exist.
    ///
    /// # Errors
    ///
    /// Propagates IO or deserialization errors from log replay
    /// ```
    pub fn open(path: impl Into<PathBuf>) -> Result<KvStore> {
        ...
        for &file in &file_list {
            let file_path = path.join(format!("{}.log", file));
    ...
    pub fn compact(&mut self) -> Result<()> {
        // increase current file by 2. current_file + 1 is for the compaction file.
        ...
        let mut new_pos = 0; // new log file position
        ...
        // remove the stale log files
        let stale_files: Vec<_> = self
            .readers
            .keys()
            .filter(|&&file| file < compaction_file)
            .cloned()
            .collect();
        for stale_file in stale_files {
            self.readers.remove(&stale_file);
            fs::remove_file(self.path.join(format!("{}.log", stale_file)))?;
        }
        self.uncompacted = 0;
        Ok(())
    }
    ...
    fn new_log_file(&mut self, file: u64) -> Result<BufWriterWithPos<File>> {
        new_log_file(&self.path, file, &mut self.readers)
    }
}
...
fn new_log_file(
    ...
    let path = path.join(format!("{}.log", file));
    ...
    readers.insert(file, BufReaderWithPos::new(File::open(&path)?)?);
    Ok(writer)
}
...
fn load(
    ...
    // Make sure we read form beginning of file
...
impl From<(u64, Range<u64>)> for CommandPos {
    fn from((idx, range): (u64, Range<u64>)) -> Self {
        CommandPos {
            idx,
            pos: range.start,
